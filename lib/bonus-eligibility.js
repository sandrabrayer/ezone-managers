/* Bonus eligibility & amount — canonical, testable logic.
   ...
   For each house the monthly bonus is decided in two parts:
     ...
     2. TREATMENT-DAYS GATE — the tier amount is only PAID if the actual
        treatment-days for the month did not fall below GATE_RATIO (100%) of the
        target. Target = tierPatients × daysInMonth.
          e.g. 17 patients in a 31-day month → target 527, min payable 527.
...
(function (root, factory) {
  ...
  /* Fraction of the treatment-days target that must be met for the monthly
     bonus to pay. 1.0 → actual treatment-days must be >= 100% of target. */
  var GATE_RATIO = 1.0;
  ...
  /* Per-house bonus configuration.
     ...
     All four houses are fully specified. Ramot is eligible from 17 patients;
   ...
   *    (the full target). Otherwise amount is 0.
   *
   ...
   *   - gatePassed:    true iff treatment-days met the full (100%) target
   ...
   *
   * Walks the house tier table from the LOWEST tier up. A tier is secured when
   * BOTH:
   *   - occupancy >= that tier's patient count, AND
   *   - daysSoFar >= GATE_RATIO (1.0) × (tierPatients × daysInMonth).
   * The walk stops at the first tier whose gate is not met: higher tiers always
   * have larger targets, so if a tier's gate fails no higher tier can pass.
   *
   * Returns the highest secured tier as
   *   { tier, amount, tierPatients, target, minRequired }
   * with all fields 0 when nothing is secured.
   */
  function securedFloor(h, resolveThreshold, daysInMonth, daysSoFar) {
    ...
    var cfg = configFor(h);
    var tiers = (cfg && Array.isArray(cfg.tiers)) ? cfg.tiers : null;
    if (!tiers || !tiers.length) return result;

    var occupancy = occupancyOf(h);
    if (occupancy < thresholdOf(h, resolveThreshold)) return result;

    var days = Number.isFinite(daysSoFar) ? daysSoFar : 0;
    var n = tiers.length;

    for (var i = n - 1; i >= 0; i--) {
      var tierPatients = tiers[i].patients;
      if (occupancy < tierPatients) break;
      var target = treatmentTarget(tierPatients, daysInMonth);
      var minRequired = GATE_RATIO * target;
      if (days < minRequired) break;
      result = {
        tier: n - i,
        amount: tiers[i].amount,
