/* Bonus eligibility & amount — canonical, testable logic.
   ...
   ── Model (Model A — patient-count tiers with a treatment-days gate) ──────────
   For each house the monthly bonus is decided in two parts:

     1. TIER AMOUNT — a step function of the END-OF-MONTH patient count
        (h.patientsNow). Each house has its own tier table. Below the house's
        eligibility threshold the tier amount is 0.
          ...
          (11 patients at the non-Ramot houses pays the 2000 tier-1 floor;
...
(function (root, factory) {
  ...
  function qualifiesMonthly(h, resolveThreshold) {
    if (!h) return false;
    return patientsNowOf(h) >= thresholdOf(h, resolveThreshold);
  }
  ...
  function monthlyBonusAmount(h, resolveThreshold, daysInMonth) {
    ...
    result.eligible = patientsNowOf(h) >= thresholdOf(h, resolveThreshold);
    result.tier = t.tier;
